...
// Terraform Registry module (terraform-aws-modules/s3-bucket/aws) — no Radius
  ...
  properties: {
    recipes: {
      'Demo.AWS/buckets': {
        ...
        recipeLocation: 'terraform-aws-modules/s3-bucket/aws:4.2.2'
        parameters: {
          // `bucket_prefix` lets the AWS provider append a unique suffix, so the
          // globally-unique bucket name never collides across runs. The prefix
          // is derived from the resource name via a {{context.*}} expression.
          bucket_prefix: '{{context.resource.name}}-'
        }
        // Map the module's outputs onto the resource's properties.
        // Keys are resource property names; values are module output names.
        outputs: {
          bucketName: 's3_bucket_id'
